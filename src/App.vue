...
/* 👇 关键优化：去除了 filter 属性 */
.main-container, .music-notify, .welcome-notify {
  /* 只过渡 opacity 和 transform，这两个开销极小 */
  transition: opacity 0.4s ease, transform 0.4s cubic-bezier(0.2, 0.8, 0.2, 1);
  opacity: 1;
  transform: scale(1);
  /* will-change: opacity, transform; <--- 不加这行，避免长期占用显存，让浏览器自己决定 */
}

.app-hidden {
  opacity: 0 !important;
  transform: scale(0.95) !important; /* 缩放幅度减小，看起来更稳 */
  pointer-events: none;
  /* ❌ 删除了 filter: blur(10px); 这是导致卡顿的元凶 */
}
